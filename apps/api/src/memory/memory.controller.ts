...
export class MemoryController {
  ...
  private async user(req: FastifyRequest): Promise<string> { const user = await this.memory.resolveIdentity(req.headers.authorization); if (!user) throw new NotFoundException('Memory disabled'); return user; }
  @Get('overview') async overview(@Req() req: FastifyRequest) { return this.memory.overview(await this.user(req)); }
