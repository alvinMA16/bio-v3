...
function Preview() {
  ...
  return <main style={{ display: 'grid', justifyItems: 'center', gap: 12, padding: 20 }}>
    ...
    <div style={{ width: 350 }}><PhonePreview activity={{ phase: option.phase, speech: option.speech, notebook: true, reducedMotion: false }} subtitle={option.label} running={false} callOpen callStartedAt={Date.now()} status={option.label} mode={attached ? 'attachment' : 'conversation'} startDisabled onStart={() => {}} onEnd={() => {}} receipt={null} receiptVisible={false} onReceiptClose={() => {}} onMaterialChat={() => {}}>{null}</PhonePreview></div>
  </main>;
}
