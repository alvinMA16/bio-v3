...
export function navigateDocumentFocus(root: HTMLElement, focus: NonNullable<DocumentView['focus']>,
  complete: (receipt: DocumentNavigationReceipt) => void, highlightRequested = false): () => void {
  const started = performance.now();
  let attempts = 0, frame = 0, timer = 0, deadline = 0, stopped = false;
  let reason: 'target_missing' | 'scroller_missing' | 'not_visible' = 'target_missing';
  ...
  const visible = (element: HTMLElement | undefined | null) => {
    ...
    return !!bounds && bounds.bottom > bounds.top && Array.from(element?.getClientRects() ?? []).some(rect =>
      rect.height > 0 && rect.bottom > bounds.top && rect.top < bounds.bottom);
  };
  const snapshot = (status: DocumentNavigationReceipt['status'], failure?: DocumentNavigationReceipt['reason']): DocumentNavigationReceipt => {
    const span = target(), rect = span?.getBoundingClientRect(), bounds = viewport();
    const mark = span?.querySelector?.<HTMLElement>('mark.document-highlight');
    return { requestId: focus.requestId, status, ...(failure ? { reason: failure } : {}), attempts,
      scrollBefore: before, scrollAfter: Math.max(0, scroller()?.scrollTop ?? 0), elapsedMs: Math.round(performance.now() - started),
      ...(rect ? { targetTop: rect.top, targetBottom: rect.bottom } : {}),
      ...(bounds ? { viewportTop: bounds.top, viewportBottom: bounds.bottom } : {}),
      highlight: !highlightRequested ? 'not_requested' : !mark ? 'missing' : visible(mark) ? 'visible' : 'not_visible' };
  };
  const cleanup = () => {
  ...
  const finish = (status: DocumentNavigationReceipt['status'], failure?: DocumentNavigationReceipt['reason']) => {
    if (stopped) return;
    cleanup(); complete(snapshot(status, failure));
  };
  ...
  const attempt = () => {
    ...
    if (!scroll) { finish('failed', 'scroller_missing'); return; }
    if (!span || !scroll.clientHeight || !span.getClientRects().length) { reason = 'target_missing'; retry(); return; }
    reason = 'not_visible';
    scroll.scrollTop += span.getBoundingClientRect().top - scroll.getBoundingClientRect().top - Math.min(80, scroll.clientHeight * .2);
    frame = requestAnimationFrame(() => {
      if (stopped) return;
      if (visible(target())) finish('visible'); else retry();
    });
  };
