...
test('panel changes and tool failures appear before final response and survive cancellation', () => {
  ...
  state = applyLiveEvent(state, event({ type: 'tool.started', name: 'update_panel_content', toolCallId: 't' }, 2), 100);
  ...
  state = applyLiveEvent(state, event({ type: 'tool.completed', name: 'update_panel_content', toolCallId: 't', isError: true }, 4), 300);
