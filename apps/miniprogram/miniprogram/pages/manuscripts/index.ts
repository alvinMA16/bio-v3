...
Page({
  ...
  load(path: string) {
    ...
    wx.request({ url: `${getApp<IAppOption>().globalData.apiBaseUrl}/agent/manuscripts${path}`, header: token ? { Authorization: `Bearer ${token}` } : {},
      success: response => { if (this.unloaded || this.generation !== generation) return; if (response.statusCode !== 200) { this.setData({ error: '无法读取文稿，请返回列表重试。' }); return; } this.setData(path ? { document: response.data as PanelDocument } : { items: response.data as Entry[], document: null }); },
