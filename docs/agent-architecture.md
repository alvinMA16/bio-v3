...
每次模型调用前按服务端当前 `scene` 注入具体行为：`conversation`（对话模式）和 `attachment_conversation`（有附件的对话模式）共用交流原则，强调共情、根据整段交流理解意图、共同推进、独立判断与事实边界。较长或较散的表达可按需精炼后继续交流，不强制总结，也不凭短回答判定结束；长度与提问服务当前意图。附件是可共同参考的资料，不预设采访流程；额外指引处理对象选择、按需读取、模型实际可见内容和原件保护。`revision` 区分建议与执行，确定目标与版本，默认轻量整理，限定修改范围并简短报告结果。场景指引是默认策略，用户明确任务优先；打开文档会进入 revision 指引，但不构成修改正文的授权。

行为回归入口见 [Agent Bench](../bench/voice/AGENT_BENCH.md)。合成案例经过实际 Agent/Pi/动态上下文链路；离线检查仅证明链路正确，真实模型回复仍需语义评审。
...
当操作会让用户等待、改变眼前内容，或遇到问题需要重试、调整做法时，prompt 要求 Agent 在当前回复内先用一句完整的简短话术说明意图，再继续执行；已说明的连续操作不重复提示，快速且不影响用户感知的操作不必逐一播报，也不通过额外模型轮次生成等待提示。Gemini 的文字与 functionCall 可共存，Pi 的 text_delta 直接进入现有 speech.delta、分句和 TTS 队列。读取或搜索执行期间可以继续播放话术；文稿修改和展示保留 beforeShow 的既有播放顺序约束。Google 内置搜索前后的 text_delta 同样可连续播报，但当前适配器不把服务商原生搜索事件转为普通 tool.started。
...
请求采集位于附件与搜索扩展之后，记录实际 Gemini SDK 参数的 HMAC 指纹、序列化字节长度、相同消息前缀、首次变化消息位置、system/tools/config/model 是否变化，以及与上次调用的间隔。这里不是原始 HTTP body，也不是 token 级前缀；SDK 内部 HTTP 重试不能逐次分辨。响应记录归一化输入和缓存 token、覆盖率、终态、总耗时和首个内容增量耗时（包括 thinking/toolcall，不等于用户听到声音的延迟）。当前 SDK 不暴露原始 usageMetadata，字段是否存在明确标记 unavailable，不将缺失冒充已证实的零。

比较基线按用户隔离的会话目录索引，仅保留哈希、字节长度，进程内最多 256 段会话、基线有效期 24 小时；重启或淘汰后无基线。超过 2048 条消息的请求跳过观测并记录原因，每 run 观测写入上限 2 MiB。指纹密钥为进程内随机密钥，不持久化。观测异步追加到现有 Trace，读取按 sequence 排序，写入失败仅告警；沿用已有 Trace 的访问控制与保存周期，不另复制正文、文件 URI 或 API key。生产镜像提供 BIO_COMMIT 标识部署版本，本地未设置时为 null。
