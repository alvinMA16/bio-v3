...
后台整理先按 `created_at, call_id` 找出每个用户最早的未完成任务，再从队首为 pending/running 且已到执行时间的用户中选取最早的 20 个候选。失败或等待重试的队首只阻塞本用户，不占用其他用户的候选名额；取得用户级 advisory lock 后再次核对队首，保持同用户串行。失败状态可通过记忆任务查询接口查看，所属用户可通过 retry 接口重试队首，成功后再处理后续任务。
