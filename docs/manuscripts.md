...
Web 文稿通话页以正文铺满背景，标题旁保留令狸小窗，正文独立滚动；底部保留紧凑通话栏和最多两行字幕，不另设专注模式。点击段落后才展示“和令狸改这段”；确认后将原文、blockId 和版本交给 Agent。语音中选择段落会打断当前轮，并在新一轮监听开始前同步选区上下文。用户仍通过 Agent 改稿，前端不直接编辑正文。
