...
将 `apps/miniprogram` 导入微信开发者工具即可运行小程序。默认首页是全屏狐狸动画场景，会依次循环播放眨眼、说话和挥手；页面不显示额外控件，业务代码可通过动画控制器切换动作。Agent 对话页仍保留为独立调试页。开发阶段需要在开发者工具中关闭域名校验；发布前把 `app.ts` 的地址替换为已备案 HTTPS 域名并加入小程序 request 合法域名。
