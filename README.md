...
已实现可选的手机号验证码登录、自动注册、30 天登录会话和退出，以及会话、记忆、资料、文稿和本地界面缓存的用户隔离。短信发送沿用 biography-v2 的阿里云 ACS3 实现。配置和部署切换见 [账号说明](docs/accounts.md)。`AUTH_ENABLED=true` 时必须配置数据库、短信签名/模板及验证码密钥；未启用时保留原私有预览模式。
